...
def main() -> None:
    ...
    calibration_info = {
        ...
        "max_patients": int(args.calibration_max_patients),
    }
    y_prob = y_prob_raw
    if args.calibrate != "none" and calibration_idx is not None and len(calibration_idx) > 0:
        ...
        calibration_info = {
            ...
            "patients": int(len(np.unique(calibration_pids))),
            "fraction": float(args.calibration_fraction),
            "max_patients": int(args.calibration_max_patients),
        }
    ...
    for thr in thresholds:
        if args.utility == "official":
            ...
            util = compute_utility(
                pid_test, hours_test, onset_test, y_test, y_prob, float(thr), alert_k=args.alert_k
            )
    ...
    alert_burden = alert_burden_stats(
        pid_test, hours_test, y_test, y_prob, best_thr, alert_k=args.alert_k
    )
